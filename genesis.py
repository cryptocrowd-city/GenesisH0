...
def get_args():
  ...
  parser.add_option("-a", "--algorithm", dest="algorithm", default="SHA256",
                    help="the PoW algorithm: [SHA256|scrypt|neoscrypt|X11|X13|X15]")
  ...
  if not options.bits:
    if options.algorithm == "scrypt" or options.algorithm == "neoscrypt" or options.algorithm == "X11" or options.algorithm == "X13" or options.algorithm == "X15":
...
def get_algorithm(options):
  supported_algorithms = ["SHA256", "scrypt", "neoscrypt", "X11", "X13", "X15"]
...
def generate_hash(data_block, algorithm, start_nonce, bits):
  ...
  while True:
    ...
    if is_genesis_hash(header_hash, target):
      if algorithm == "X11" or algorithm == "X13" or algorithm == "X15" or algorithm == "neoscrypt":
